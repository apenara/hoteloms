...
export default function HousekeepingLoginPage() {
  ...
  // Si ya está autenticado, redirigir a la vista de housekeeping
  useEffect(() => {
    if (staff?.role === "housekeeping") {
      router.push(`/housekeeping/${params.hotelId}/${staff.id}`);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const currentStaff = JSON.parse(
        sessionStorage.getItem("currentStaff") ||
          localStorage.getItem("currentStaff") ||
          "{}"
      );
      router.push(`/housekeeping/${params.hotelId}/${currentStaff.id}`);
    } catch (error) {
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-2">
          <CardTitle className="text-2xl flex items-center gap-2">
            ...
            {hotel.name || "Acceso Housekeeping"}
          </CardTitle>
          <CardDescription>
            Ingresa con tu PIN de personal de limpieza
          </CardDescription>
        </CardHeader>
        <CardContent>
