...
const calcularEficienciaHabitacion = (
  ...
  const tiempoEsperado =
    tiemposEsperados[habitacion.status as keyof typeof tiemposEsperados] ||
    tiemposEsperados.default;
...
export const getDuracionEstimada = (tipoLimpieza: string): number => {
  ...
  return (
    duraciones[tipoLimpieza as keyof typeof duraciones] || duraciones.default
  );
};
